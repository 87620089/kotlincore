 ...
 * https://github.com/easyandroidgroup/EasyAndroid/blob/master/utils/src/main/java/com/haoge/easyandroid/easy/EasyLog.kt
 */
...
//        if(buffer.length>1024*10||result.toString().contains("FFFD3800000000000000".toRegex())) {
            FileIOUtils.writeFileFromString(File(mCurrentPath), result.toString(), true)
...
//        }
    ...
    //
    private var mCurrentPath = Environment.getExternalStorageDirectory().path
    fun initFilePath(filePath: String) {
        mCurrentPath = filePath
        val nameString =
            SimpleDateFormat("yyyy_MM_dd HH:mm:ss.SSS ", Locale.getDefault()).format(Date())

        val file = File(mCurrentPath, nameString.substring(0, 10) + ".txt")

//        if(file.exists()){
//            Log.i(tag,"已经存在"+file.absolutePath)
//        }else{
//            val boolean=file.mkdirs()
//            Log.i(tag,"不存在，创建"+file.absolutePath+"--boolean="+boolean)
//        }
